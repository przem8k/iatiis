...
\paragraph*{}
In effect we obtain two images (or one two-channel image), representing vertical and horizontal derivative at each pixel. Combining these two components into vectors representing two-dimensional gradient yields results demonstrated in \reffig{GradientVectors}, where the resulting gradient vectors were drawn onto the original image.
...
\begin{refImpl}
Gradient operators discussed in this section are implemented by \studio filter \filter{GradientImage\_Mask}{ImageLocalTransform}.
\end{refImpl}